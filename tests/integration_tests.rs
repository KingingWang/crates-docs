...
async fn test_cache_functionality() {
    ...
    // 测试缓存过期
    cache.set("expiring_key".to_string(), "expiring_value".to_string(), Some(std::time::Duration::from_secs(1))).await;
...
fn test_server_creation() {
    ...
    assert!(server_result.is_ok(), "服务器创建失败: {:?}", server_result.err());
    ...
    assert!(server_info.capabilities.tools.is_some(), "服务器应该提供工具能力");
}
...
fn test_oauth_config() {
    ...
    assert_eq!(config.redirect_uri, Some("http://localhost:8080/oauth/callback".to_string()));
