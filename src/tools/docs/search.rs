...
pub struct SearchCratesTool {
    ...
    /// 结果数量限制
    #[json_schema(title = "结果限制", description = "返回的最大结果数量", minimum = 1, maximum = 100, default = 10)]
    pub limit: Option<u32>,
    
    /// 输出格式
    #[json_schema(title = "输出格式", description = "搜索结果输出格式", default = "markdown")]
...
impl SearchCratesToolImpl {
    ...
    /// 搜索 crate
    async fn search_crates(&self, query: &str, limit: u32) -> std::result::Result<Vec<CrateInfo>, CallToolError> {
        // 构建缓存键
        ...
        // 发送 HTTP 请求
        let response = self.service.client()
            .get(&url)
        ...
        if !response.status().is_success() {
            return Err(CallToolError::from_message(
                format!("搜索失败，状态码: {}", response.status())
            ));
        }
        ...
        self.service.cache()
            .set(cache_key, cache_value, Some(std::time::Duration::from_secs(300)))
            .await;
...
fn parse_crates_response(json: &serde_json::Value, limit: usize) -> Vec<CrateInfo> {
    ...
    if let Some(crates_array) = json.get("crates").and_then(|c| c.as_array()) {
        for crate_item in crates_array.iter().take(limit) {
            let name = crate_item.get("name")
                .and_then(|n| n.as_str())
                .unwrap_or("Unknown")
                .to_string();
            
            let description = crate_item.get("description")
                .and_then(|d| d.as_str())
                .map(std::string::ToString::to_string);
            
            let version = crate_item.get("max_version")
                .and_then(|v| v.as_str())
                .unwrap_or("0.0.0")
                .to_string();
            
            let downloads = crate_item.get("downloads")
                .and_then(serde_json::Value::as_u64)
                .unwrap_or(0);
            
            let repository = crate_item.get("repository")
                .and_then(|r| r.as_str())
                .map(std::string::ToString::to_string);
            
            let documentation = crate_item.get("documentation")
                .and_then(|d| d.as_str())
                .map(std::string::ToString::to_string);
    ...
    match format {
        ...
        "markdown" => {
            ...
            for (i, crate_info) in crates.iter().enumerate() {
                ...
                writeln!(output, "**Docs.rs**: [https://docs.rs/{}/](https://docs.rs/{}/)\n",
                    crate_info.name, crate_info.name).unwrap();
            }
...
impl Tool for SearchCratesToolImpl {
    ...
    async fn execute(&self, arguments: serde_json::Value) -> std::result::Result<rust_mcp_sdk::schema::CallToolResult, rust_mcp_sdk::schema::CallToolError> {
        let params: SearchCratesTool = serde_json::from_value(arguments)
            .map_err(|e| rust_mcp_sdk::schema::CallToolError::invalid_arguments("search_crates", Some(format!("参数解析失败: {e}"))))?;
