...
impl OAuthConfig {
    ...
    pub fn keycloak(
        ...
        Self {
            ...
            client_secret: Some(client_secret),
            redirect_uri: Some(redirect_uri),
            authorization_endpoint: Some(format!("{base}/realms/{realm}/protocol/openid-connect/auth")),
            token_endpoint: Some(format!("{base}/realms/{realm}/protocol/openid-connect/token")),
            scopes: vec!["openid".to_string(), "profile".to_string(), "email".to_string()],
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if let Some(endpoint) = &self.authorization_endpoint {
            Url::parse(endpoint).map_err(|e| Error::Config(format!("无效的 authorization_endpoint: {e}")))?;
        }
        
        if let Some(endpoint) = &self.token_endpoint {
            Url::parse(endpoint).map_err(|e| Error::Config(format!("无效的 token_endpoint: {e}")))?;
        }
